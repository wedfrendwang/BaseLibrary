...
//            mWholeHeaderView.setBackgroundColor(getResources().getColor(android.R.color.white,null));
//        }else{
            mWholeHeaderView.setBackgroundColor(Color.parseColor("#ffffff"));

//        }
        //对于这个添加方法，在做界面加载的时候已经说过了。
    ...
    public void setRefreshViewHolder() {
        mRefreshViewHolder = new WeLiveRefreshHolder(context,true);
    ...
    private void initRefreshHeaderView() {
        ...
        if (mRefreshHeaderView != null) {
            ...
            mMinWholeHeaderViewPaddingTop = -(int)(mRefreshHeaderViewHeight*mRefreshViewHolder.getSpringDistanceScale());
    ...
    /**
     * 项目中需求显示，是否主动进行界面的刷新显示，并且显示下拉刷新的动画
     */
    public void startBeginRefreshing(){
        if(mRecyclerView!=null){
            if(mRecyclerView.getLayoutManager() instanceof LinearLayoutManager){
                LinearLayoutManager linearLayoutManager = (LinearLayoutManager) mRecyclerView.getLayoutManager();
                int position = linearLayoutManager.findFirstVisibleItemPosition();
                if(position == 0){//表示该RecyclerView处于最顶层，可以执行该刷新的方法
                    beginRefreshing();
                }
            }
    ...
    public void beginRefreshing() {
        if (mCurrentRefreshStatus != RefreshStatus.REFRESHING && mDelegate != null) {
            ...
            handleRefreshStatusChanged();
            mDelegate.onRefreshLayoutPullRefreshing(this);
        }
    }
    ...
    /**
     * 开始上拉加载更多，会触发delegate的下拉加载方法
     */
    public void beginLoadingMore() {
        if (!mIsLoadingMore && mLoadMoreFooterView != null && mDelegate != null && mDelegate.onRefreshLayoutLoadingMore(this)) {
...
//        RefreshScrollingUtil.scrollToBottom(mAbsListView);
    ...
    public interface RefreshLayoutDelegate {
         ...
         * 开始刷新
         */
        void onRefreshLayoutPullRefreshing(RefreshLayout refreshLayout);
        ...
        boolean onRefreshLayoutLoadingMore(RefreshLayout refreshLayout);
    }
